            ...
            for(int x=0;x<mapTileWidth;x++)
                {
                    if(p_map[y][x]==solidTile)
                                    {
                                        chunk.push_back(tileMap(greenBrick,vector2f(tileSize*x+(chunkNumber+chunkPosition)*screenWidth,tileSize*y),solidTile));
                                    }
                    else if(p_map[y][x]==platformer)
                                    {
                                        chunk.push_back(tileMap(platform,vector2f(tileSize*x+(chunkNumber+chunkPosition)*screenWidth,tileSize*y),platformer));
                                        if(p_map[y][x-1]==1)
                                        {
